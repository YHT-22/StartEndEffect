...
for f0Index = 1:length(f0)
    ...
    for f1Index = 1:length(f1)
        ...
        ChangeDuration = ChangePeriod * (1 / f1(f1Index));%sec
        ...
        if any(Change_End > (TotalDuration - rfTime) * 1000)
            error("Change in rise-fall time!Please check 'ChangePeriod'.");
        end
        wave = y1_addrisefall;
        filenames = rowFcn(@(x, y) [ord(n_Freq + y, :), ...
                                    '_f0-', num2str(f0(f0Index)), ...
                                    '_DiffRatio-', num2str(f1(f1Index) / f0(f0Index)), ...
                                    '_nChangePeriod-', num2str(ChangePeriod), ...
                                    '_ChangeDur-', num2str(ChangeDuration * 1000), 'ms', ...
